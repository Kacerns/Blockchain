...
//             int Temp_index = gen() % sender.utxos.size();
//             UTXO utxo_to_spend = sender.utxos[Temp_index];

//             int amount_to_send = gen() % utxo_to_spend.amount + 1;
//             if(utxo_to_spend.amount > sender.balance()){
//                 cout<<"balance error 1"<<endl;
//                 cout<<sender.balance();
//                 cout <<utxo_to_spend.amount<<endl;
//                 cout << amount_to_send <<endl;
//                 break;
//             }
//             if(amount_to_send > sender.balance()){
//                 cout<<"balance error 2"<<endl;
//                 cout<<sender.balance();
//                 cout <<utxo_to_spend.amount<<endl;
//                 cout << amount_to_send <<endl;
//                 break;
//             }
//             if(amount_to_send > utxo_to_spend.amount){
//                 cout<<"balance error 3"<<endl;
//                 cout<<sender.balance();
//                 cout <<utxo_to_spend.amount<<endl;
//                 cout << amount_to_send <<endl;
//                 break;
...
#include <sstream>
#include <random>
#include "einsteinhash.hpp"  // Assume this file contains the hash function
...
vector<Transaction> generate_transactions(Blockchain &blockchain, const vector<User> &users, int target_num_transactions) {
    ...
    while (transactions.size() < target_num_transactions) {
        ...
        for (const auto &pair : blockchain.utxo_pool) {
            if (pair.second.owner == sender.public_key) {
        ...
        if (!sender_utxos.empty()) {
            ...
            if (utxo_to_spend.amount > amount_to_send) {
                UTXO change_utxo(utxo_to_spend.tx_id + "_change", utxo_to_spend.amount - amount_to_send, sender.public_key);
                outputs.push_back(change_utxo);
            }
